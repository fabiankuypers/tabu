...
const RoomDetailsModal: React.FC<RoomDetailsModalProps> = ({
  ...
  }, [isOpen, showPhotoPreview, onClose]);
  ...
  const handleImageClick = () => {
    setPhotoPreviewIndex(selectedImageIndex);
  ...
  const handlePhotoPreviewNavigate = (direction: 'next' | 'previous') => {
    if (direction === 'next') {
      setPhotoPreviewIndex((prev) => (prev + 1) % gallery.length);
    } else {
      setPhotoPreviewIndex((prev) => (prev - 1 + gallery.length) % gallery.length);
    }
  };
  ...
  return (
    <>
            ...
            <div className="relative h-96 md:h-[28rem] lg:h-[32rem]">
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all duration-300 flex items-center justify-center">
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/80 backdrop-blur-sm px-4 py-2 rounded-full border border-slate-700/50">
                    <div className="flex items-center text-white text-sm">
                      <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7" />
                      </svg>
                      {language === 'de' ? 'Vollbild ansehen' : 'View Fullscreen'}
                    </div>
                  </div>
                </div>
              </button>
              ...
              {/* Navigation Arrows - Only for current room */}
              {gallery.length > 1 && (
                <>
                  <button
                    onClick={(e) => {
                ...
                <div className="flex gap-3 overflow-x-auto pb-2">
                  {gallery.map((img, index) => (
                    <button
                      ...
                      className={`flex-shrink-0 w-20 h-20 rounded-xl overflow-hidden border-2 transition-all cursor-zoom-in group relative ${
      ...
      {/* Photo Preview Modal (Second Tier) */}
      <PhotoPreviewModal
        images={gallery}
        currentIndex={photoPreviewIndex}
        isOpen={showPhotoPreview}
        onClose={handlePhotoPreviewClose}
        onNavigate={handlePhotoPreviewNavigate}
      />
    </>
  );
};
